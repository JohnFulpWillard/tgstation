...
/obj/item/restraints/legcuffs/bola/cult/attack_hand(mob/living/carbon/user, list/modifiers)
	...
	if(user.num_legs < 2 || !isnull(carbon_user.equipped_items_by_slot["[ITEM_SLOT_LEGCUFFED]"])) //if they can't be ensnared, stun for the same time as it takes to breakout of bola
		to_chat(user, span_cult_large("\"I wouldn't advise that.\""))
		user.dropItemToGround(src, TRUE)
